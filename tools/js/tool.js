...
Use a more recent file?`)) {
  ...
  saveAsFile: function() {
    ...
    var blob = new Blob([this.getCode(true)], {
...
Point.prototype.select = function(value, invert) {
  ...
  if (selected && !this.selected) {
    ...
    if (camera.trackingMap && !parent.document.hasFocus()) {
      Tour.view.set({
        id: select.points[0].panorama.id
      })
    }
...
Point.prototype.mouseDown = function(event) {
  ...
  if (!event.altKey) {
    ...
    select.points.forEach(function(point) {
      ...
      if (that.panorama.links && that.panorama.links.find(function(n) {
          return n.id == point.panorama.id
        })) {
        point.panorama.links = point.panorama.links.filter(function(n) {
          return n.id != that.panorama.id
        })
        that.panorama.links = that.panorama.links.filter(function(n) {
          return n.id != point.panorama.id
        })
        links.setPoints()
      } else {
        if (!point.panorama.links) point.panorama.links = [];
        point.panorama.links.push({
          id: that.panorama.id
        })

        if (!that.panorama.links) that.panorama.links = [];
        that.panorama.links.push({
          id: point.panorama.id
        })
      }
    })
    links.draw();
...
Point.prototype.mouseMoveRotate = function(event) {
  ...
  if (event.which == 1) {
    ...
    if (Tour.view.id == this.panorama.id) {
      if (event.altKey) {
        Tour.view.set({
          lon: -this.panorama.heading - this.cameraShift
        })
      } else {
        this.cameraShift = -this.panorama.heading - Tour.view.lon.value
      }
...
var markers = {
  ...
  add: function(action, direction, icon, id) {
    ...
    markers.push({
      action: action || {
        type: "popup",
        id: prompt('marker id:', panorama.markers ? panorama.markers.length : 0)
      },
...
areas = {
  init: function() {
    ...
    this.domElement.addEventListener('changeOrder', ({
      detail: {
        from,
        to
      }
...
AreaPoint.prototype.mouseMove = function(event) {
  var absolute = areaEditor.absoluteToArea({
    x: event.pageX -  this.mouse.x,
    y: event.pageY -  this.mouse.y
  })
  this.x = absolute.x;
  this.y = absolute.y;
  areaEditor.draw();
...
areaEditor.push = function(event) {
  if (event.which == 1) {
    this.points.push(new AreaPoint(areaEditor.absoluteToArea({
      x: event.pageX,
      y: event.pageY
    })));
    this.set();
...
utils = {
  ...
  updatePanoRotate: function() {
    ...
    Tour.markers.forEach(function(m) {
      m.setVector()
    })
    Tour.needsUpdate = true;
  },
    ...
    if (select.points.length) {
      ...
      select.points.forEach(function(a) {
        select.points.forEach(function(b) {
          panoLinks.push({
            distance: utils.getDistance(a, b),
            a: a,
            b: b
          })
        })
      ...
      panoLinks.sort(function(a, b) {
        ...
        if (utils.checkFreeLinks(p.a, p.b)) {
          p.a.panorama.links.push({
            id: p.b.panorama.id
          })
          p.b.panorama.links.push({
            id: p.a.panorama.id
          })
        }
      })
    ...
    points.forEach(function(point) {
      if (point.panorama.links) point.panorama.links.forEach(function(link) {
        if (point.panorama.id > link.id && link.x != undefined && link.y != undefined) {
          new ConnectiPoint(link, [link, utils.findPoinnt(link.id).panorama.links.find(function(l) {
            return l.id == point.panorama.id
          })]);
        }
      })
    })
    ...
    points.forEach(function(point) {
      if (point.visible && point.panorama.links && point.panorama.links.length) {
        point.panorama.links.forEach(function(link) {
          ...
          var linkA = link;
          var linkB = b.panorama.links.find(function(l) {
            return l.id == a.panorama.id
          });
...
var uploadYandex = function() {
  var CSRFTOKEN = ''

  var position = (new URLSearchParams(location.search)).get('ll').split(',').map(n => parseFloat(n)).reverse();
  ...
  var LAT = 1111945;
  var api = 'https://pano.maps.yandex.ru/api/'
  tourData = JSON.parse(tourData);
  var firstPano = getByTourId(tourData.panorams[0].id) 
  position[1] -= (firstPano.x / 10 / LAT);
  position[0] += (firstPano.y / 10 / LAT);
  var skinData = {}
  ...
  function getPanoramas() {
    fetch(api + 'getPanoramas', {
      ...
      if (data.err) {
        alert(data.err.statusCode + ' ' + data.err.error + '\n ' + data.err.message)
      } else {
        panorams = JSON.parse(JSON.stringify(data.data.items));
        yandexData = data.data.items;
        tourData.panorams.forEach(function(pano) {
          if (pano.links) {
            pano.links.forEach(function(link) {
              links.push({
                dstId: getPanoByFileName(link.id).id,
                shipmentId: shipmentId,
                srcId: getPanoByFileName(pano.id).id,
              })
            })
          }
        })
  ...
  function updatePanorams() {
    var pano = panorams.shift();
    if (pano) {
      updatePanorama(pano, function() {
        updatePanorams();
      });
    } else {
      addLinks()
  ...
  fetch = function() {
    ...
    var token = args[1].headers['X-Csrf-Token'];
    var n = token + '1'
    if (token.length && !CSRFTOKEN) {
      console.log(token);
      CSRFTOKEN = token;
      activeShipment.click()
      getPanoramas();
    }
    return f.apply(this, args);
  };

  var activeShipment = document.querySelector('.shipments-list__content .list-item.list-item_highlight-color_blue');
  var otherShipment = document.querySelector('.shipments-list__content .list-item:not(.list-item_highlight-color_blue)');
  otherShipment.click()
}
...
var copyCodeToUploadToPanoSkin = function() {
  copyTextToClipboard("var tourData = '" + JSON.stringify(Tour.data) + "';" + uploadPanoSkin.toString().slice(12, -1));
  window.open('https://login.panoskin.com');
}
var copyCodeToUploadToYandex = function() {
  copyTextToClipboard("var tourData = '" + JSON.stringify(Tour.data) + "';" + uploadYandex.toString().slice(12, -1));
  window.open('https://pano.maps.yandex.ru');
}
