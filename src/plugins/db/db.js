class DB {
  ...
  getBy(table, key, value) {
    return table.find((r) => r[key] == value);
  }
  ...
  getByHash(table, hash) {
    return this.getBy(table, "_id", hash);
  }

  getLangKey(table = [], key, options) {
    const record = table.find((r) => r.key === key);
    const string = record
      ? record[Lang.language] || record["en"] || record["ru"]
      : key;
    return this.prepareLangString(string, options);
  }
  ...
  langRender() {
    document.querySelectorAll("body *[data-lang]").forEach((element) => {
      element.innerHTML = this.getLangKey(
        this.tables.langs,
  ...
  load() {
    ...
    Promise.all(
      ...
      responses.forEach((table, index) => {
        table.getBy = (key, value) => this.getBy(table, key, value);
      ...
      if (db.tables.langs) {
        Lang.getKey = (key, options) =>
          this.getLangKey(this.tables.langs, key, options);
